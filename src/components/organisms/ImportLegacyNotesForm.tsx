...
import { useLocalDB } from '../../lib/v2/hooks/useLocalDB'
...
const ImportLegacyNotesForm = ({ storageId }: ImportLegacyNotesFormProps) => {
  ...
  const { createFolderApi, createDocApi } = useLocalDB()
    ...
    try {
      ...
      for (const [, folderName] of folderMap) {
        await createFolderApi({
          ...
          destinationPathname: destinationFolderPathname,
        }).catch((err) => {
          pushMessage({
            title: 'Error',
      ...
      for (const note of convertedNotes) {
        ...
        await createDocApi({ workspaceId: storageId, docProps: note })
      }
